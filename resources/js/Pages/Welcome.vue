        ...
        <header class="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-100">
            <nav class="mx-auto flex max-w-7xl items-center justify-between p-4 lg:px-8">
                ...
                <div class="flex lg:hidden">
                    <button type="button" 
                        class="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-slate-700 hover:bg-slate-100 transition" 
                ...
                <div class="hidden lg:flex lg:gap-x-12">
                    <a href="#fitur" class="text-sm font-semibold leading-6 text-slate-600 hover:text-blue-600 transition-colors">Layanan</a>
                    <Link :href="route('panduan')" class="text-sm font-semibold leading-6 text-slate-600 hover:text-blue-600 transition-colors">Panduan</Link>
                    <Link :href="route('kontak')" class="text-sm font-semibold leading-6 text-slate-600 hover:text-blue-600 transition-colors">Kontak</Link>
                </div>
                ...
                <div class="hidden lg:flex lg:flex-1 lg:justify-end lg:gap-x-4">
                    ...
                    <template v-else>
                        <Link :href="route('login')">
                            <Button variant="ghost" class="font-semibold text-slate-600">Masuk</Button>
                        </Link>
                        <Link :href="route('register')">
                            <Button class="bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-lg shadow-blue-500/30">
                                Daftar Akun
                            </Button>
                        </Link>
                    </template>
            ...
            <div v-if="isMenuOpen" class="lg:hidden" role="dialog" aria-modal="true">
                <div class="fixed inset-0 z-[90] bg-black/20 backdrop-blur-sm" @click="isMenuOpen = false"></div>
                
                <div class="fixed inset-y-0 right-0 z-[100] w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm border-l shadow-2xl transition-transform">
                    <div class="flex items-center justify-between">
                        <span class="text-xl font-bold text-slate-900">Menu</span>
                        <button type="button" class="-m-2.5 rounded-md p-2.5 text-slate-700 hover:bg-slate-100" @click="isMenuOpen = false">
                    ...
                    <div class="mt-6 flow-root">
                        <div class="-my-6 divide-y divide-slate-100">
                            <div class="space-y-2 py-6">
                                <a href="#fitur" class="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50" @click="isMenuOpen = false">
                                    Layanan
                                </a>
                                <Link :href="route('panduan')" class="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50" @click="isMenuOpen = false">
                                    Panduan
                                </Link>
                                <Link :href="route('kontak')" class="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50" @click="isMenuOpen = false">
                                    Kontak
                                </Link>
                            </div>
                            <div class="py-6 flex flex-col gap-3">
                                ...
                                <template v-else>
                                    <Link :href="route('login')"><Button variant="outline" class="w-full justify-center">Masuk</Button></Link>
                                    <Link :href="route('register')"><Button class="w-full bg-blue-600 hover:bg-blue-700 text-white">Daftar Akun</Button></Link>
                                </template>
                            </div>
                        </div>
                    </div>
            ...
            <div class="relative isolate pt-24 lg:pt-32 pb-16 overflow-hidden lg:mt-8 xs:mt-0">
                <div class="mx-auto max-w-7xl px-6 lg:px-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                        ...
                        <div class="max-w-2xl">
                            <Badge variant="outline" class="mb-6 px-4 py-1 text-sm border-blue-200 bg-blue-50 text-blue-700 rounded-full">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4">
                                <template v-if="$page.props.auth.user">
                                    <Link :href="route('dashboard')">
                                        <Button size="lg" class="h-12 px-8 text-base bg-blue-600 hover:bg-blue-700 shadow-xl shadow-blue-500/20 w-full sm:w-auto">
                            ...
                            <Card class="absolute top-0 right-0 w-[80%] h-[280px] bg-white shadow-2xl border-slate-100 rounded-2xl z-10 transform translate-x-8 -translate-y-8">
                                ...
                                <CardContent>
                                    <div class="flex items-end justify-between h-32 gap-2 mt-4">
                                        ...
                                        <div class="w-full bg-blue-500 rounded-t-sm h-[70%]"></div>
                                        <div class="w-full bg-blue-200 rounded-t-sm h-[50%]"></div>
                                        <div class="w-full bg-blue-100 rounded-t-sm h-[30%]"></div>
                                        <div class="w-full bg-blue-300 rounded-t-sm h-[60%]"></div>
                                        <div class="w-full bg-blue-400 rounded-t-sm h-[80%]"></div>
                            ...
                            <Card class="absolute bottom-10 left-0 w-[85%] h-[220px] bg-slate-900 text-white shadow-2xl border-none rounded-2xl z-20 overflow-hidden">
                                ...
                                <CardContent class="mt-4 space-y-4">
                                    <div class="flex items-center gap-4 bg-white/5 p-3 rounded-lg border border-white/10">
                                        <div class="p-2 bg-blue-500 rounded-full"><FileText class="h-5 w-5 text-white" /></div>
                                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <Card class="border-none shadow-sm hover:shadow-xl transition-all duration-300">
                            <CardHeader>
                                ...
                                <CardTitle>Data Terintegrasi</CardTitle>
                            </CardHeader>
                            <CardContent class="text-slate-600">
                                Data kependudukan Anda aman dan terpusat, memudahkan proses verifikasi tanpa perlu input ulang berkali-kali.
                            </CardContent>
                        </Card>
